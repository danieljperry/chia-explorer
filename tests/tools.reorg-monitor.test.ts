...
describe('reorg monitor tools (MCP)', () => {
  let client: Client;
  ...
  it('start_reorg_monitor returns started=true', async () => {
    mockPeak(1000, 'a'.repeat(64));
    mockBlockRecords([makeBlockRecord(1000, 'a'.repeat(64))]);

    const res = await client.callTool({
      name: 'start_reorg_monitor',
      arguments: { poll_interval_seconds: 10, lookback_blocks: 3 },
    });
    const body = parseToolText(res) as { started: boolean; poll_interval_seconds: number };
    expect(body.started).toBe(true);
    expect(body.poll_interval_seconds).toBe(10);
  });

  it('get_reorg_monitor_status reports active after start', async () => {
    mockPeak(2000, 'b'.repeat(64));
    mockBlockRecords([makeBlockRecord(2000, 'b'.repeat(64))]);
    startMonitor({ poll_interval_seconds: 10, lookback_blocks: 3, network: 'mainnet' });

    const res = await client.callTool({ name: 'get_reorg_monitor_status', arguments: {} });
    const body = parseToolText(res) as { active: boolean; network: string };
    expect(body.active).toBe(true);
    expect(body.network).toBe('mainnet');
  });

  it('stop_reorg_monitor returns stopped=true and active=false', async () => {
    mockPeak(3000, 'c'.repeat(64));
    mockBlockRecords([makeBlockRecord(3000, 'c'.repeat(64))]);
    startMonitor({ poll_interval_seconds: 10, lookback_blocks: 3, network: 'mainnet' });

    const res = await client.callTool({ name: 'stop_reorg_monitor', arguments: {} });
    const body = parseToolText(res) as { stopped: boolean; active: boolean };
    expect(body.stopped).toBe(true);
    expect(body.active).toBe(false);
  });

  it('start_reorg_monitor returns isError when startMonitor throws', async () => {
    vi.spyOn(monitorModule, 'startMonitor').mockImplementationOnce(() => {
  ...
  // ── zod schema validation on alert_recipients ──

  it('rejects a malformed email in alert_recipients', async () => {
    const res = (await client.callTool({
      name: 'start_reorg_monitor',
      arguments: {
        poll_interval_seconds: 10,
        lookback_blocks: 3,
        alert_recipients: [{ email: 'not-an-email', min_blocks: 1 }],
      },
    })) as { isError?: boolean };
    expect(res.isError).toBe(true);
  });

  it('rejects min_blocks: 0 in alert_recipients', async () => {
    const res = (await client.callTool({
      name: 'start_reorg_monitor',
      arguments: {
        poll_interval_seconds: 10,
        lookback_blocks: 3,
        alert_recipients: [{ email: 'u@example.com', min_blocks: 0 }],
      },
    })) as { isError?: boolean };
    expect(res.isError).toBe(true);
  });

  it('rejects more than 10 alert_recipients', async () => {
    ...
    const res = (await client.callTool({
      ...
      arguments: {
        poll_interval_seconds: 10,
        lookback_blocks: 3,
        alert_recipients: tooMany,
      },
    })) as { isError?: boolean };
    expect(res.isError).toBe(true);
  });

  it('defaults min_blocks to 1 when omitted', async () => {
    mockPeak(1000, 'a'.repeat(64));
    mockBlockRecords([makeBlockRecord(1000, 'a'.repeat(64))]);
    const res = await client.callTool({
      name: 'start_reorg_monitor',
      arguments: {
        poll_interval_seconds: 10,
        lookback_blocks: 3,
        alert_recipients: [{ email: 'a@example.com' }],
      },
    });
    const body = parseToolText(res) as {
      alert_recipients: Array<{ email: string; min_blocks: number }>;
    };
    expect(body.alert_recipients).toHaveLength(1);
    expect(body.alert_recipients[0]!.min_blocks).toBe(1);
  });

  it('deduplicates alert_recipients by email, keeping the first occurrence', async () => {
    mockPeak(1000, 'a'.repeat(64));
    mockBlockRecords([makeBlockRecord(1000, 'a'.repeat(64))]);
    const res = await client.callTool({
      name: 'start_reorg_monitor',
      arguments: {
        ...
        lookback_blocks: 3,
        alert_recipients: [
          { email: 'dupe@example.com', min_blocks: 1 },
          { email: 'dupe@example.com', min_blocks: 3 },
          { email: 'other@example.com', min_blocks: 2 },
        ],
      },
    });
    const body = parseToolText(res) as {
    ...
    expect(body.alert_recipients).toHaveLength(2);
    expect(body.alert_recipients[0]).toEqual({ email: 'dupe@example.com', min_blocks: 1 });
    expect(body.alert_recipients[1]).toEqual({ email: 'other@example.com', min_blocks: 2 });
  });

  it('propagates an explicit network argument through start_reorg_monitor', async () => {
    mockPeak(1000, 'a'.repeat(64));
    mockBlockRecords([makeBlockRecord(1000, 'a'.repeat(64))]);
    const res = await client.callTool({
      name: 'start_reorg_monitor',
    ...
    const body = parseToolText(res) as { network: string };
